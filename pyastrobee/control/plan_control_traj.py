...
Akshara Rai wrote parts of this code for our CoRL2019 paper.
...
def plan_linear_quaternion_trajectory(q0, q_goal, N):
    amount = np.linspace(0.0, 1.0, num=N)
    traj = np.zeros((N, q0.shape[0]))
    for i in range(N):
        h = amount[i]
        traj[i] = (1-h)*q0 + h*q_goal
...
def plan_linear_position_trajectory(p0, p_goal, N):
    amount = np.linspace(0., 1., num=N)
    traj = np.repeat((p_goal - p0)[None], N, axis=0) * amount[:, None] + p0
    return traj
...
def plan_control_traj(target_pos, target_quat, num_steps, freq, curr_pos, curr_quat, curr_vel=None, ori_type='quat'):
    ...
    # Y = plan_linear_position_trajectory(curr_pos, target_pos, num_steps)
    th = plan_linear_orientation_trajectory(
        curr_orient, ee_orient, num_steps)  # 3 euler angles (custom)
    if ori_type == 'quat':
