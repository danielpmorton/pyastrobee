"""Testing out methods to stabilize the bag at the end of the trajectory

IDEAS
- MPC with a bunch of possible stabilization trajectories
- get the current state of the bag (not realistic in the ISS, but a good starting point)
- use the state of the bag to figure out some sort of proportional/derivative term
- use the PD term to decide what motion/forces should be applied at the handle location
- Generate a bunch of possible trajectories to do this
- Rollout MPC and evaluate which is "best"... best defined by the state of the bag. Maybe add some stuff to the 
  cost function based on the robot state.. but we can also always disconnect the robot and then stabilize that after
...
def bag_stabilization_mpc(
    ...
    # Give the cargo bag an initial velocity/angular velocity
    np.random.seed(0)  # Should this be in the environments' seed parameters?
    lin_vel_stdev = 0.2
    ang_vel_stdev = 0.1
    lin_vel = np.random.normal(0, lin_vel_stdev, size=3)
    ang_vel = np.random.normal(0, ang_vel_stdev, size=3)
    main_env.client.resetBaseVelocity(main_env.bag.id, lin_vel, ang_vel)
    bag_ids = vec_env.get_attr("bag.id")
    clients = vec_env.get_attr("client")
    for bag_id, client in zip(bag_ids, clients):
        client.resetBaseVelocity(bag_id, lin_vel, ang_vel)

    # Generate nominal trajectory
    dt = main_env.client.getPhysicsEngineParameters()["fixedTimeStep"]

    # For the bag stabilization task, plan a trajectory that should help counter the motion of the bag
    # The MPC might need a different structure overall than the trajectory following task
    # TODO see if there is a way to generalize the MPC function to serve multiple use cases??

    pos, orn, vel, ang_vel = main_env.robot.dynamics_state

    # Assume that we're working with the top handle bag
    # If we characterize it as a rigid body we can use the known dimensions to the handle to determine the force
    # that we'd need to apply to stabilize the rotational motion
    # Could also model it as a pendulum??

    goal_pos = NotImplemented
    goal_orn = NotImplemented
    goal_vel = NotImplemented
    goal_ang_vel = NotImplemented
...
def _test_bag_stabilization():
    ...
    n_vec_envs = 5
    debug = True
    bag_stabilization_mpc(start_pose, end_pose, duration, n_vec_envs, debug)


if __name__ == "__main__":
    _test_bag_stabilization()
