...
from pyastrobee.utils.camera_utils import cameraConfig
...
class ProcessCamera:
    ...
    def make_pcd(
        ...
        if object_ids is not None:
            if segment_mask is None:
                raise ValueError(
                    "Segmentation mask (segment_mask) should be specified if object ids are provided"
                )
            else:
        ...
        horizontal = np.array(config.cam_horiz)
        vertical = np.array(config.cam_vert)
        ...
        A_ortho = np.vstack(
            [horizontal / width, -vertical / height, np.zeros(3), constant]
        ).T
        ...
        # -- depth
        depth_v = (
            far_val * near_val / (far_val - (far_val - near_val) * input_homogeneous[2])
        )
        # -- res
    ...
    def draw_point_clouds_from_file(fname_pfx, max_num_pts=None):
        ...
        for step in range(0, max_steps + 1, draw_every):
            ...
            if step == 0:
                print("ptcloud", pts.shape, "tracking_ids", ids.shape)
            if pts.shape[0] == 0:
                continue  # no pts to plot
            print(
                "Draw step {:d} num_pts {:d} {:s}".format(
                    step,
                    pts.shape[0],
    ...
    def render(
        ...
        w, h, rgba_px, depth_raw_cam_dists, segment_mask = sim.getCameraImage(
            width=width,
            height=height,
            viewMatrix=config.view_matrix,
            projectionMatrix=config.proj_matrix,
            shadow=False,
        ...
        if debug:
            print("make_pcd took {:4f}".format(time.time() - tic))
    ...
    def draw_point_clouds(pts: np.ndarray, ids: np.ndarray, cmap=plt.cm.jet):
        ...
        ax.scatter(
            pts[:, 0], pts[:, 1], pts[:, 2], marker=".", s=2.0, c=ids[:, 0], cmap=cmap
        )
        ax.set_xlim(-0.4, 1.0)
        ax.set_ylim(-0.6, 0.6)
        ax.set_zlim(0, 0.8)
