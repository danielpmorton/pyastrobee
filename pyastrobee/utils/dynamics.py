...
def state_matrix(
    ...
    w: npt.ArrayLike,
    inertia: np.ndarray,
    inv_inertia: Optional[npt.ArrayLike] = None,
) -> np.ndarray:
    ...
    Args:
        ...
        inv_inertia (Optional[npt.ArrayLike]): Inverse of the inertia tensor. This should be precomputed ahead of time
            for efficiency. Defaults to None (compute on the fly)
...
def control_matrix(mass: float, inv_inertia: np.ndarray) -> np.ndarray:
